...
namespace tml
{
	...
	CallModuleEvent::CallModuleEvent(Module& module) :
		ModuleEvent(module, module.order) { }

	// 准备调用逻辑链中Module的mainFunction

	int __stdcall MyOnCallLogicModule(Module* pModule)
	{
		CallModuleEvent event_(*pModule);
		if (g_eventBus.Post(THLogicEvent::OnCallLogicModule, event_))
			event_.result = -1;
	...
	__declspec(naked) void MyOnCallLogicModuleWrapper()
	{
		__asm
		{
			push edi
			call MyOnCallLogicModule
			cmp eax, -1
			jne Cancel
			// 没取消，交给原函数
			jmp g_onCallLogicModuleHook.m_newEntry
	...
	InlineHook g_onCallLogicModuleHook((void*)0x40155F, MyOnCallLogicModuleWrapper, 6);
	...
	{
		bool res = true;
		res = res && g_onCallLogicModuleHook.IsEnabled();
